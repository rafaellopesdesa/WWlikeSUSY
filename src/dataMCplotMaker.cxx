...
float AdjustedMaximum(int side, std::vector <TH1D*> Plots, TH1D* data = null, std::vector <TH1D*> Signals = std::vector<TH1D*>()){
  int lowerBound = 1;
...
TCanvas* dataMCplotMaker(TH1D* Data, std::vector <TH1D*> Backgrounds, std::vector <string> Titles, std::string titleIn, std::string title2In, std::string options_string, std::vector <TH1D*> Signals, std::vector <string> SignalTitles, std::vector <Color_t> color_input){
  ...
  if (noData == false){
    finPad[1]->cd();
    TH1D* err_hist = (TH1D*)Backgrounds[0]->Clone(); 
    err_hist->SetTitle("");
    err_hist->Draw();
